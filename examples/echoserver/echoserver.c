        ...
        while (ChildRunning) {
                    ...
                    if (rc == WS_CHAN_RXD) {
                        ...
                        if (lastChannel == agentChannelId) {
                            ...
                            if (cnt_r <= 0)
                                ...
                                buf_dump(channelBuffer, cnt_r);
                            #endif
                            cnt_w = (int)write(agentFd, channelBuffer, cnt_r);
                            if (cnt_w <= 0)
                                break;
                        }
                        #endif
                        ...
                        if (lastChannel == fwdChannelId) {
                            ...
                            if (cnt_r <= 0)
                                ...
                                buf_dump(channelBuffer, cnt_r);
                            #endif
                            cnt_w = (int)write(fwdFd, channelBuffer, cnt_r);
                            if (cnt_w <= 0)
                                break;
                        }
                        #endif
            ...
            if (threadCtx->agentCbCtx.state == AGENT_STATE_CONNECTED) {
                if (FD_ISSET(agentFd, &readFds)) {
                    ...
                    #endif
                    cnt_r = (int)read(agentFd, agentBuffer, sizeof agentBuffer);
                    if (cnt_r == 0) {
            ...
            if (threadCtx->fwdCbCtx.state == FWD_STATE_CONNECTED) {
                if (FD_ISSET(fwdFd, &readFds)) {
                    ...
                    #endif
                    cnt_r = (int)read(fwdFd, fwdBuffer + fwdBufferIdx,
                            sizeof fwdBuffer - fwdBufferIdx);
                    if (cnt_r == 0) {
                        /* Read zero-returned. Socket is closed. Go back
                           to listening. */
